...
public class Shrapnel extends ThrowableProjectile {
    ...
    public Shrapnel(Level level) {
        super(NtmEntityTypes.SHRAPNEL.get(), level);
    }
        ...
        if(this.tickCount > 5) {
            ...
            if(b == 2 || b == 4) {
                if(!level.isClientSide) {
                    ...
                    if(this.getDeltaMovement().y > 0 && result instanceof BlockHitResult bhr) {
                        ...
                        ExplosionVNT vnt = new ExplosionVNT(level(), pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, 7)
                                ...
                                .setBlockProcessor(new BlockProcessorStandard().setNoDrop().withBlockEffect(new BlockMutatorDebris(NtmBlocks.VOLCANIC_LAVA.get())));
                        vnt.explode();
                    }
                }
