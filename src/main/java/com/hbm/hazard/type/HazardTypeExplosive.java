...
public class HazardTypeExplosive extends HazardTypeBase {
    ...
    public void onUpdate(LivingEntity target, float level, ItemStack stack) {
        ...
        if (!target.level().isClientSide && target.isOnFire() && stack.getCount() > 0) {
            stack.setCount(0);
            target.level().explode(null, target.getX(), target.getY(2), target.getZ(), 1.0F, Level.ExplosionInteraction.NONE);
        }
    }
    ...
    public void updateEntity(ItemEntity item, float level) {
        ...
        if (item.isOnFire() || item.isInLava()) {
            item.discard();
            item.level().explode(null, item.getX(), item.getY(2), item.getZ(), 1.0F, Level.ExplosionInteraction.NONE);
        }
    }
