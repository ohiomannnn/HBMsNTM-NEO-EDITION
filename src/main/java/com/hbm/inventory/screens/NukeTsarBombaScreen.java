...
public class NukeTsarBombaScreen extends InfoScreen<NukeTsarBombaMenu> {
    ...
    private static final ResourceLocation TEXTURE = NuclearTechMod.withDefaultNamespace("textures/gui/weapon/tsar_bomba_schematic.png");
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        for(String s : I18nUtil.resolveKeyArray("container.nuke_tsar_bomba.desc")) desc.add(Component.literal(s));
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if(be.isFilled()) guiGraphics.blit(NukeIvyMikeScreen.TEXTURE, this.leftPos + 18, this.topPos + 50, 176, 0, 16, 16);
        if(be.isReady()) guiGraphics.blit(NukeIvyMikeScreen.TEXTURE, this.leftPos + 18, this.topPos + 50, 176, 18, 16, 16);

        for(int index = 0; index < 4; index++) {
            if(be.slots.get(index).is(NtmItems.EXPLOSIVE_LENSES.get())) {
                switch(index) {
        ...
        if(be.slots.get(5).is(NtmItems.TSAR_BOMBA_CORE.get())) guiGraphics.blit(NukeIvyMikeScreen.TEXTURE, this.leftPos + 75 + 16, this.topPos + 25 + 16, 176, 220, 80, 36);
